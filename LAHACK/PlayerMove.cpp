...
void PlayerMove::create_final_ques() {
	mOwner->SetState(ActorState::Paused);
	created = true;
	cout << "creating final actor" << endl;
	Actor* actor_1 = new Actor(mOwner->GetGame());
	SpriteComponent* sc_1 = new SpriteComponent(actor_1);
	sc_1->SetTexture(mOwner->GetGame()->GetTexture("Assets/ques_1.png"));
	actor_1->SetPosition(Vector2(150, 45 + (level-1) * 448));

	words = new Words(mOwner->GetGame());
	words->SetPosition(Vector2(0, 100 + (level - 1) * 448));
	words->binj = "DRAWER";
	words->subj = "LETTER";
}
...
void PlayerMove::Update(float deltaTime) {
	...
	Show_Hide(mOwner->GetGame()->bed->GetComponent<CollisionComponent>(), tel[3], 41, 2);
	Show_Hide(mOwner->GetGame()->shelf->GetComponent<CollisionComponent>(), tel[4], 27, 2);
	...
	if (mOwner->GetGame()->cnt_collect == 1) {
		Vector2 thanos = mOwner->GetGame()->thanos;
		boss = new Thanos(mOwner->GetGame());
		//set position
		boss->SetPosition(Vector2(300, 2640 - 14));
	}


	//checking whether the player colldies with the thanos
	if (boss) {
		CollisionComponent* thanos_cc = boss->GetComponent<CollisionComponent>();
		Vector2 offset = Vector2(0, 0);
		if (playerCC->GetMinOverlap(thanos_cc, offset) != CollSide::None) {
			if (playerCC->GetMinOverlap(thanos_cc, offset) == CollSide::Top) {
				mYSpeed = 0.0f;
				mInAir = false;
			}
			notColliding = false;
