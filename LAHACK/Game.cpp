...
bool Game::Initialize() {
	...
	if (!font) {
		std::cout << "incorrect font loading" << std::endl;
	}
...
void Game::UpdateGame() {
	...
	//setting Tip
	if (start_tip) {
		my_tip->time -= deltaTime;
		if (my_tip->time <= 0.0f) {
			player->SetState(ActorState::Active);
			my_tip->SetState(ActorState::Active);
			for (size_t i = 0; i < goombas.size(); i++) {
				goombas[i]->SetState(ActorState::Active);
			}
			my_tip->SetState(ActorState::Destroy);
			my_tip = nullptr;
			start_tip = false;
		}
	}
...
void Game::LoadData() {
	...
	background->SetPosition(Vector2(3392, 224 * 5));
	...
	cc4->SetSize(224, 32);
	door_4 = new Teleport(this, 3, 32, 32);
	door_4->SetPosition(Vector2(245, 2530));

	shelf = new Actor(this);
	shelf->SetPosition(Vector2(512, 2384));
	CollisionComponent* cc5 = new CollisionComponent(shelf);
	cc5->SetSize(128, 32);
	door_5 = new Teleport(this, 2, 32, 32);
	door_5->SetPosition(Vector2(520, 2270));
		...
		while (getline(myReadFile, line)) {
			for (int i = 0; i < line.length(); i++) {
				...
				if (current == "A") {
					...
					thanos = (Vector2(centerX - 16, centerY - 8));
				}
				centerX += BlockWidth;
			}
			centerX = 16;
...
SDL_Surface * Game::GetSurface(string words) {
	if (surfaceMap.find(words) != surfaceMap.end()) {
		return surfaceMap[words];
	}
	else {
		SDL_Surface * surface = TTF_RenderText_Solid(font,
			words.c_str(), { 255, 0, 0 });
		surfaceMap[words] = surface;
		return surface;
	}
}
std::string Game::FileChanged(std::string fileName) {
...
void Game::WriteFile(std::string objA, std::string objB, std::string words, std::string fileName) {
	std::ofstream output(fileName);
	std::string temp;
	output << objA << "," << objB << std::endl;
	output << words;
	output.close();
}